...
class Cluster:
    ...
    def avgDistance(self):
        ...
        for i in range(len(self.points)):
            selflistpo = list(self.points)
            #print('p', selflistpo[i])
            selflistcen = list(self.center)
            dist = math.dist(selflistpo[i], selflistcen)
            distli.append(dist)
        avedist = sum(distli) / len(distli)
        return avedist
    ...
    def updateCenter(self):
        ...
        for i in range(self.center.dim):
            ...
            for j in (self.points):
                coords.append(j[i])
                self.center.coords[i] = sum(coords) / len(self.points)
                #meanv = statistics.mean(xy)
                #p.append(meanv)
            #self.center = Point(p)
            #print (xy)
            #return self.center
